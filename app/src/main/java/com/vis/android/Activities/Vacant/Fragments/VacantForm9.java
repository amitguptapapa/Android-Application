...
public class VacantForm9 extends BaseFragment implements View.OnClickListener {
...
//            rlPBDGMake, rlPBDGCapacity, rlPBDGCondition;
    LinearLayout llNotAvailableWithin, llAvailableWithin, llhvac, llGarden, llPowerB,llArea;
    TextView next, tvPrevious;
    Intent intent;
    Boolean edit_page = true;

    String   garBeauty, garOrdinary;

    int spinGarden = 0, spinPBCon = 0, spinPB = 0,spinArea=0;
    String cbJetPumpCheck,cbSubmersibleCheck,cbJalBoardSupplyCheck,cbTubeWellsCheck;

    CheckBox cbGarBeautiful, cbGarOrdinary,cbJetPump,cbSubmersible,cbJalBoardSupply,cbTubeWells;


    //RadioGroup
    RadioGroup rgBoundaryWall,rgArea,rgConnection,rgCurrentAct,
    //            rgGarden,
    rgParking,
            rgAvailableWithin, rgNotAvailableWithin;

    //EditText
    EditText etBoundaryRunningMtr, etBoundaryHeight, etBoundaryWidth,etArea,
...
//    int cbInverterCheck,cbDgSetCheck, cbOnGroundCheck,cbInBasementCheck,cbOnStiltCheck,cbOnRoadCheck,cbAcuteParkingCheck,cbNoPowerBackupCheck;
    ...
    Spinner  spinnerGarden,  spinnerType, spinnerFinish,
    //            spinnerPBInverterCondition,spinnerPBDGCondition,
    spinnerSecurityHut, spinnerPavements, spinnerBoundaryHeightFtMtr,spinnerArea;
...
//    LinearLayout llInverter,llDGSet;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        v = inflater.inflate(R.layout.frag_vacantform9, container, false);
...
//        progressbar.setProgress(8);
    ...
    public void setEditiblity() {
        ...
        cbTubeWells.setClickable(false);

        rgBoundaryWall.setClickable(false);
        rgArea.setClickable(false);
        rgConnection.setClickable( false );
        rgCurrentAct.setClickable( false );
...
*/
        ...
        for (int i = 0; i < rgArea.getChildCount(); i++)
            rgArea.getChildAt(i).setClickable(false);
        for (int i = 0; i < rgConnection.getChildCount(); i++)
            rgConnection.getChildAt(i).setClickable(false); 
        for (int i = 0; i < rgCurrentAct.getChildCount(); i++)
            rgCurrentAct.getChildAt(i).setClickable(false);
...
//            Toast.makeText(getActivity(), "Scheduled Case", Toast.LENGTH_SHORT).show();
       ...
       /* rlLiftMake = (RelativeLayout) v.findViewById(R.id.rlLiftMake);*/
        ...
        rgArea = v.findViewById(R.id.rgArea);
        etArea = v.findViewById(R.id.etArea);
        llArea = v.findViewById(R.id.llArea);
        rgConnection = v.findViewById(R.id.rgConnection);
        rgCurrentAct = v.findViewById(R.id.rgCurrentAct);
...
//        rgGarden = v.findViewById(R.id.rgGarden);
       ...
       /* etLiftCapacity = v.findViewById(R.id.etLiftCapacity);*/
        ...
        cbTubeWells = v.findViewById(R.id.cbTubeWells);
...
//        });
        ...
        cbJetPump.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
        ...
        cbSubmersible.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
        ...
        cbJalBoardSupply.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
        ...
        cbTubeWells.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
...
//                                                           hitGetAreaMeasurementApi(pref.get(Constants.case_id),property_array_list.get(i).get("id"));
        ...
        spinnerArea.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                                                    @Override
                                                    public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                                                        spinArea = spinnerArea.getSelectedItemPosition();
//                                                           Log.v("shapeSelected", shapeSelected);
                                                        }
...
//                                                           hitGetAreaMeasurementApi(pref.get(Constants.case_id),property_array_list.get(i).get("id"));
            ...
            case R.id.next:
                if (edit_page)
                    validation();
                        ...
                        ((Dashboard) mActivity).displayView(27);
    ...
    /*    makePB = etPowerMake.getText().toString();
        ...
        if (rgBoundaryWall.getCheckedRadioButtonId() == -1) {
            ...
            Snackbar snackbar = Snackbar.make(next, "Please Select Rooms", Snackbar.LENGTH_SHORT);
            snackbar.show();
            View targetView = v.findViewById(R.id.rgArea);
            targetView.getParent().requestChildFocus(targetView, targetView);

        } else if (((RadioButton) rgArea.getChildAt(0)).isChecked() && etArea.getText().toString().isEmpty()) {
            Snackbar snackbar = Snackbar.make(next, "Please Enter Rooms Area", Snackbar.LENGTH_SHORT);
            snackbar.show();
            etArea.requestFocus();
        }if (rgConnection.getCheckedRadioButtonId() == -1) {
            Snackbar snackbar = Snackbar.make(next, "Please Select Connections", Snackbar.LENGTH_SHORT);
            snackbar.show();
            View targetView = v.findViewById(R.id.rgConnection);
            targetView.getParent().requestChildFocus(targetView, targetView);

        }if (rgCurrentAct.getCheckedRadioButtonId() == -1) {
            Snackbar snackbar = Snackbar.make(next, "Please Select Current Activity", Snackbar.LENGTH_SHORT);
            snackbar.show();
            View targetView = v.findViewById(R.id.rgCurrentAct);
            targetView.getParent().requestChildFocus(targetView, targetView);
...
//        }
      ...
      /*  hm.put("PBoption", Integer.toString(spinPB));
        ...
        hm.put("cbJetPumpCheck",cbJetPumpCheck);
        hm.put("cbSubmersibleCheck",cbSubmersibleCheck);
        hm.put("cbJalBoardSupplyCheck",cbJalBoardSupplyCheck);
        hm.put("cbTubeWellsCheck",cbTubeWellsCheck);
     ...
     /*   if (cbNotAvailableWithin.isChecked()){
        ...
        int selectedIdrgArea = rgArea.getCheckedRadioButtonId();
        View radioButtonrgArea = v.findViewById(selectedIdrgArea);
        int idx1 = rgArea.indexOfChild(radioButtonrgArea);
        RadioButton r1 = (RadioButton) rgArea.getChildAt(idx1);
        String selectedText1 = r1.getText().toString();
        hm.put("radioButtonRooms", selectedText1);
        hm.put("radioButtonRoomspos", Integer.toString(idx1));
        hm.put("radioButtonRoomsArea", etArea.getText().toString());
        hm.put("Areaoption", Integer.toString(spinArea));
        int selectedIdrgConnection = rgConnection.getCheckedRadioButtonId();
        View radioButtonrgConnection = v.findViewById(selectedIdrgConnection);
        int idx2 = rgConnection.indexOfChild(radioButtonrgConnection);
        RadioButton r2 = (RadioButton) rgConnection.getChildAt(idx2);
        String selectedText2 = r2.getText().toString();
        hm.put("radioButtonConnection", selectedText2);
        hm.put("radioButtonConnectpos", Integer.toString(idx2));
        
        
        
        int selectedIdrgCurrentAct = rgCurrentAct.getCheckedRadioButtonId();
        View radioButtonrgCurrentAct = v.findViewById(selectedIdrgCurrentAct);
        int idx3 = rgCurrentAct.indexOfChild(radioButtonrgCurrentAct);
        RadioButton r3 = (RadioButton) rgCurrentAct.getChildAt(idx3);
        String selectedText3 = r3.getText().toString();
        hm.put("radioButtonCurrentAct", selectedText3);
        hm.put("radioButtonCurrentActpos", Integer.toString(idx3));
...
*/
                if (object.has("Gardenoption")) {
                    ...
                    spinGarden = 0;
                if (object.has("cbJetPumpCheck")) {
                   if(object.getString("cbJetPumpCheck").equalsIgnoreCase( "1" ))
                    cbJetPump.setChecked( true );
                    else
                    cbJetPump.setChecked( false );
                }if (object.has("cbTubeWellsCheck")) {
                   if(object.getString("cbTubeWellsCheck").equalsIgnoreCase( "1" ))
                       cbTubeWells.setChecked( true );
                    else
                       cbTubeWells.setChecked( false );
                }
                if (object.has("cbSubmersibleCheck")) {
                   if(object.getString("cbSubmersibleCheck").equalsIgnoreCase( "1" ))
                       cbSubmersible.setChecked( true );
                    else
                       cbSubmersible.setChecked( false );
                } if (object.has("cbJalBoardSupplyCheck")) {
                   if(object.getString("cbJalBoardSupplyCheck").equalsIgnoreCase( "1" ))
                ...
                if (object.has("garOrdinary")) {
                    ...
                    garOrdinary = "";
                if (object.has("Areaoption")) {
                    spinArea = Integer.parseInt(object.getString("Areaoption"));
                    spinnerArea.setSelection(spinArea);
                } else
                    spinArea = 0;
...
*/
           ...
           /*     if (object.getString("cbOnRoaad").equals("1")){
                ...
                if(object.has("radioButtonRoomspos")){
                    int idx =Integer.parseInt(object.getString("radioButtonRoomspos"));
                    ((RadioButton) rgArea.getChildAt(idx)).setChecked(true);
                    if(idx==0){
                        llArea.setVisibility(View.VISIBLE);
                        etArea.setText(object.getString("radioButtonRoomsArea"));
                    }
                }
                if(object.has("radioButtonConnectpos")){
                    int idx =Integer.parseInt(object.getString("radioButtonConnectpos"));
                    ((RadioButton) rgConnection.getChildAt(idx)).setChecked(true);
                } if(object.has("radioButtonCurrentActpos")){
                    int idx =Integer.parseInt(object.getString("radioButtonCurrentActpos"));
                    ((RadioButton) rgCurrentAct.getChildAt(idx)).setChecked(true);
                }
            ...
            public boolean onMenuItemClick(MenuItem item) {
                switch (item.getItemId()) {
                    ...
                    case R.id.general_ten:
                            ((Dashboard) mActivity).displayView(26);
                        return true;
                    default:
                        return false;
                }
